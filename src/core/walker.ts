...
const groupFilesByComponentName = (files: string[]): FilesMap => (
  files.reduce<FilesMap>((acc, filePath) => {
...
const resolveFilesPath = (src: string, filesMap: FilesMap): FilesMap => (
  Object.entries(filesMap).reduce<FilesMap>((acc, [componentName, componentFiles]) => ({
    ...acc,
 ...
 */
export const getComponentFiles = async (source: string, names: string[] = [], extensions: string[] = ['tsx']): Promise<FilesMap> => {
  const pattern = `**/*.${extensions.join(',')}`
  const patterns = names.length > 0
    ? names.map((name) => `${name}/${pattern}`)
    : [pattern]
